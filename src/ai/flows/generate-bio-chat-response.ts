'use server';
/**
 * @fileOverview Manages a chatbot conversation to help a user build their professional work repository.
 *
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert career coach and resume writer. You are having a conversation with a user to help them build their professional work repository (experience, skills, achievements) from scratch.

**Your Task:**
1.  Analyze the user's latest message in the chat history.
2.  Based on their message, update the "Current Work Repository" text. You must integrate their new information, rewrite sections as requested, or add new sections. The result should always be a single, coherent block of text representing the complete repository.
...
-   **Always return the FULL, updated work repository.** Do not just return the changed part.
-   If the user pastes a large block of text (like a resume), your primary job is to parse it and structure it into the repository. Your response should confirm that you've processed it.
...
**Current Work Repository (to be updated):**
---
{{{currentWorkRepository}}}
---
