...
'use server';
/**
 * @fileOverview Generates a structured CV from a user's work repository, tailored to a job description.
 *
...
- For a missing full name, use '[Name not found in repository]'.
- For ALL other missing fields (email, phone, location, job title, company, summary, etc.), use the exact string '[Information not found in repository]'.
- DO NOT use words like "Unknown" or "N/A". Only use the specified placeholders.

Based on the user's work repository and the job description, generate the following sections:
...
3.  **Work Experience**: For each job mentioned in the repository, extract the job title, company, duration of employment, and create a list of 3-5 bullet points describing key responsibilities and achievements. Tailor the language to match keywords from the job description.
4.  **Education**: Extract all educational qualifications, including degree, institution, and year of graduation.
5.  **Skills**: Create a list of the most relevant technical and soft skills from the user's repository that align with the job description.
