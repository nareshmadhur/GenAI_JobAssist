...
'use server';
/**
 * @fileOverview Analyzes a user's professional work repository for completeness.
 *
 * - analyzeBioCompleteness - A function that checks for key sections in a repository.
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert resume analyzer. Your task is to check a user's professional work repository for the presence of key sections.

For each of the following sections, determine if it is present and adequately represented in the repository. Be critical: a single sentence mentioning a skill might not be enough to count for the 'Skills' section if it's not a clear list.
...
Analyze the following work repository now and return the boolean flags indicating the presence of each section.
